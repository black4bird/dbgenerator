  ...
  module Realm
    module Java
      ...
      class Generator
        ...
        def initialize(path, package_name, xcdatamodel)
          puts "\n"
          ...
          xcdatamodel.entities.each do |_, entity|
            unless entity.abstract?
              ...
              generate_class(path, package_name, entity)
            end
          end
        end
        ...
        def generate_attributes(class_file, attributes, relationships, primary_key)
          # "NORMAL" ATTRIBUTES
        ...
        def write_attributes(attributes, primary_key)
          ...
          attributes.each_with_index do |(_, attribute), idx|
            unless attribute.read_only?
              name = attribute.name
              type = attribute.enum? ? 'String' : convert_type(attribute.type)
              if type
                # Realm annotations
              ...
              getters_and_setters_string << generate_getter_and_setter(type, name)
              getters_and_setters_string << "\n" + generate_enum_getter_and_setter(attribute.enum_type.delete_objc_prefix, name) if attribute.enum?
        ...
        def generate_getter_and_setter(type, name)
          ...
          getter_setter << '    ' + '}' + "\n\n"
          getter_setter << '    ' + 'public void set' + name.capitalize_first_letter + '(final '+ type + ' ' + name + ') {' + "\n"
